...
export function extendInventoryCategories(categories = []) {
  ...
  for (const extra of EXTRA_INVENTORY_CATEGORIES) {
    ...
    const armorIndex = next.findIndex((item) => item.value === "armor");
    ...
    else if (extra.value === "tools") insertAt = magazineIndex >= 0 ? magazineIndex + 1 : (beverageIndex >= 0 ? beverageIndex + 1 : next.length);
    next.splice(insertAt, 0, extra);
  }
  return next;
}

const aidItems = aidRows.map(([name, effect, duration, addiction, weight, cost, rarity]) => ({ name, category: "aid", effect, duration, addiction, weight, cost, rarity }));
const toConsumable = (category) => ([name, healing, effect, radiation, weight, cost, rarity]) => ({ name, category, healing, effect, radiation, weight, cost, rarity });
...
export const INVENTORY_DATABASE = [
  ...
  ...RARE_BOOK_ITEMS,
  ...toolItems,
  ...SETTLERS_GUIDE_TOOL_ITEMS,
  ...SETTLERS_GUIDE_ROBOT_PARTS,
  ...BOBBLEHEAD_ITEMS,
...
export function getInventoryArchiveItems(category) {
  ...
  return INVENTORY_DATABASE.filter((item) => item.category === category).map((item) => ({
    ...item,
    canonicalName: item.name,
    canonicalEffect: item.effect,
    name: item.localizedName?.[language] || item.localizedName?.en || translateInventoryItemName(item.name, language),
    effect: item.localizedEffect?.[language] || item.localizedEffect?.en || translateInventoryItemEffect(item.effect, language),
  }));
}
