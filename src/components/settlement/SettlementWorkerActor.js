...
export class SettlementWorkerActor {
  constructor(scene, index = 0) {
    ...
    this.symbol = scene.add.text(0, -43, '', { fontSize: '13px', color: '#ffe1a0', stroke: '#10251a', strokeThickness: 3 }).setOrigin(.5);
    ...
    this.container.add([this.shadow, this.left, this.right, this.body, this.arm, this.head, hat, this.tool, this.cargo, this.symbol, this.label]);
    this.body.setInteractive();
    this.body.on('pointerover', () => this.label.setVisible(true));
    this.body.on('pointerout', () => { if (!this.pinned) this.label.setVisible(false); });
    this.body.on('pointerdown', (_pointer, _x, _y, event) => {
  ...
  configure(world, job, { resident, actionName = '', targetName = '', language = 'en', reduced = false, reset = false }) {
    ...
    this.copy = COPY[String(language).split('-')[0]] || COPY.en;
  ...
  render(time) {
    ...
    this.container.setDepth(20 + this.container.y / 10000 + (this.pinned ? 1 : 0));
    ...
    if (caption !== this.caption) {
      ...
      this.symbol.setText(state.phase === 'waiting' ? '…' : state.cargo ? '▣' : SYMBOLS[action] || '');
