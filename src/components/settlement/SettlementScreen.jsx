...
export default function SettlementScreen({ settlement, character=null, onUpdate, onBack, onCommand, canEdit=true, sharedControls, payment, canClaimProfit=false }) {
  ...
  const rawSelectedRequirementBlockers=selectedRule ? constructionRequirementBlockers(character,selectedRule) : [];
  ...
  return <div className={`pip-screen settlement-screen settlement-dashboard settlement-v2 ${onCommand ? 'is-shared-settlement' : ''} ${settlement.offlineStandalone ? 'is-offline-settlement' : ''} ${panelOpen ? 'is-panel-open' : ''} ${editorMode ? 'is-editor-mode' : ''}`}>
    ...
    <div className="settlement-layout settlement-dashboard-grid">
      ...
      <aside className="settlement-summary settlement-right-panel pip-panel" aria-label={ui[panelMode] || text[panelMode]}>
        ...
        {panelMode==="build" && !selectedBuilding ? <><div className="settlement-build-categories" role="tablist" aria-label={text.build}>{BUILD_CATEGORIES.map(category=><button key={category} type="button" className={selectedCategory===category ? "is-selected" : ""} onClick={()=>{setSelectedCategory(category);setSelectedType(null);setHoverCell(null);setNotice("");}}>{categoryLabels[category]}</button>)}</div><div className="settlement-build-menu">{visibleBuildings.map(def=>{const asset=getSettlementAsset(def.asset);const rule=getRulebookBuilding(def.id);const rawBlockers=constructionRequirementBlockers(character,rule);const blockers=rawBlockers.filter(blocker=>!(contractorRank>=2 && blocker.kind==="skill"));const family=rule?.effects?.storeFamily;const duplicateStore=family==="medical" ? (settlement.buildings || []).some(building=>building.state!=="destroyed" && getRulebookBuilding(building.type)?.effects?.storeFamily==="medical") : family==="trading" ? !nextStoreSpecialty(settlement) : false;return <button key={def.id} type="button" disabled={!canEdit || blockers.length>0 || duplicateStore} className={selectedType===def.id ? "is-selected" : ""} onClick={()=>{setSelectedType(def.id);setStoredBuildingId(null);setSelectedBuildingId(null);setMovingBuildingId(null);setNotice("");setPanelOpen(false);}}><div className="settlement-build-menu__preview">{asset ? <img src={asset} alt=""/> : <span>{BUILDING_ICONS[def.id] || "⌂"}</span>}</div><span>{settlementBuildingName(def,language)}</span><small>{def.footprint.width}×{def.footprint.height} · {formatRulebookCost(rule)} · {rule.constructionDays}d{blockers.length ? ` · ⚠ ${blockers.map(blocker=>blocker.label || blocker.kind).join(", ")}` : duplicateStore ? " · ⚠ STORE" : ""}</small></button>;})}</div></> : null}
        {panelMode==="build" ? <>
