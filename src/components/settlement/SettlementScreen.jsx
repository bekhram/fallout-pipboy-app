...
export default function SettlementScreen({ settlement, character=null, onUpdate, onBack, onCommand, canEdit=true, sharedControls, payment, canClaimProfit=false }) {
  ...
  async function createBuildingAt(x,y) {
    ...
    if(family && (settlement.buildings || []).some(building=>building.state!=="destroyed" && getRulebookBuilding(building.type)?.effects?.storeFamily===family)){setNotice("STORE_TYPE_EXISTS");return;}
