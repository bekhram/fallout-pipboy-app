...
import { buildingUpgradeInfo, buildBlockers, cost, startUpgrade, upgradeRule } from '../../utils/settlementDevelopment.js';
import SettlementDailySummary from "./SettlementDailySummary.jsx";
import SettlementEconomyPanel from "./SettlementEconomyPanel.jsx";
...
function readSettlementReputationRank(settlement){
  if(typeof window==="undefined")return 2;
  try{
    const store=JSON.parse(window.localStorage.getItem("pip2d20_settlement_reputations_v2")||"null");
    const rows=Array.isArray(store?.rows)?store.rows:[];
    const id=String(settlement?.id||""),name=String(settlement?.name||"").trim().toLowerCase();
    const row=rows.find(item=>(id&&String(item?.sourceId||"")===id)||String(item?.name||"").trim().toLowerCase()===name);
    return Math.max(0,Math.min(5,Number(row?.rank??2)));
  }catch{return 2;}
}
function addMarketInventoryItem(items,item,quantity=1){
  const next=structuredClone(items||[]),key=String(item?.canonicalName||item?.name||"").trim().toLowerCase();
  const existing=next.find(entry=>String(entry?.canonicalName||entry?.name||"").trim().toLowerCase()===key&&String(entry?.category||"")===String(item?.category||""));
  if(existing){const qty=Math.max(0,Number(existing.quantity??existing.qty??1))+quantity;existing.quantity=String(qty);if(Object.hasOwn(existing,"qty"))existing.qty=qty;return next;}
  next.push({...item,canonicalName:item.canonicalName||item.name,quantity:String(quantity)});
  return next;
}
...
export default function SettlementScreen({ settlement, onUpdate, onBack, onCommand, canEdit=true, sharedControls, payment, canClaimProfit=false, onRemoveGuestNpc, ownerCharacter=null, setOwnerCharacter=null }) {
  ...
  const [tradeAmounts,setTradeAmounts]=useState({common:1,uncommon:1,rare:1});
  const ui=({en:{overview:'Overview',title:'Settlement',more:'More',chat:'Chat',close:'Close panel',choose:'Select a building on the map',zoomIn:'Zoom in',zoomOut:'Zoom out',economy:'Economy'},ru:{overview:'Обзор',title:'Поселение',more:'Ещё',chat:'Чат',close:'Закрыть панель',choose:'Выберите здание на карте',zoomIn:'Приблизить',zoomOut:'Отдалить',economy:'Экономика'},uk:{overview:'Огляд',title:'Поселення',more:'Ще',chat:'Чат',close:'Закрити панель',choose:'Оберіть будівлю на мапі',zoomIn:'Збільшити',zoomOut:'Зменшити',economy:'Економіка'},pl:{overview:'Przegląd',title:'Osada',more:'Więcej',chat:'Czat',close:'Zamknij panel',choose:'Wybierz budynek na mapie',zoomIn:'Przybliż',zoomOut:'Oddal',economy:'Ekonomia'}})[language] || {overview:'Overview',title:'Settlement',more:'More',chat:'Chat',close:'Close panel',choose:'Select a building on the map',zoomIn:'Zoom in',zoomOut:'Zoom out',economy:'Economy'};
  const tradeText=({en:{title:'MARKET',closed:'Assign a worker to a trading building to open the market.',tier:'Market tier',buy:'BUY',sell:'SELL',buyPrice:'Buy',sellPrice:'Sell',level:'Level',next:'Next level',upgrade:'UPGRADE',upgradeCost:'Upgrade cost',upgradeTime:'Upgrade time',changes:'Improves',caps:'caps',daily:'DAILY STOCK',merchant:'Merchant',barter:'Barter',rep:'Reputation',left:'left'},ru:{title:'РЫНОК',closed:'Назначьте работника в торговое здание, чтобы открыть рынок.',tier:'Уровень рынка',buy:'КУПИТЬ',sell:'ПРОДАТЬ',buyPrice:'Покупка',sellPrice:'Продажа',level:'Уровень',next:'Следующий уровень',upgrade:'УЛУЧШИТЬ',upgradeCost:'Стоимость улучшения',upgradeTime:'Время улучшения',changes:'Улучшает',caps:'крышек',daily:'АССОРТИМЕНТ НА ДЕНЬ',merchant:'Торговец',barter:'Бартер',rep:'Репутация',left:'осталось'},uk:{title:'РИНОК',closed:'Призначте працівника до торгової будівлі, щоб відкрити ринок.',tier:'Рівень ринку',buy:'КУПИТИ',sell:'ПРОДАТИ',buyPrice:'Купівля',sellPrice:'Продаж',level:'Рівень',next:'Наступний рівень',upgrade:'ПОКРАЩИТИ',upgradeCost:'Вартість покращення',upgradeTime:'Час покращення',changes:'Покращує',caps:'кришок',daily:'АСОРТИМЕНТ НА ДЕНЬ',merchant:'Торговець',barter:'Бартер',rep:'Репутація',left:'залишилось'},pl:{title:'RYNEK',closed:'Przydziel pracownika do budynku handlowego, aby otworzyć rynek.',tier:'Poziom rynku',buy:'KUP',sell:'SPRZEDAJ',buyPrice:'Kupno',sellPrice:'Sprzedaż',level:'Poziom',next:'Następny poziom',upgrade:'ULEPSZ',upgradeCost:'Koszt ulepszenia',upgradeTime:'Czas ulepszenia',changes:'Ulepsza',caps:'kapsli',daily:'DZIENNY ASORTYMENT',merchant:'Handlarz',barter:'Barter',rep:'Reputacja',left:'pozostało'}})[language] || {title:'MARKET',closed:'Assign a worker to a trading building to open the market.',tier:'Market tier',buy:'BUY',sell:'SELL',buyPrice:'Buy',sellPrice:'Sell',level:'Level',next:'Next level',upgrade:'UPGRADE',upgradeCost:'Upgrade cost',upgradeTime:'Upgrade time',changes:'Improves',caps:'caps',daily:'DAILY STOCK',merchant:'Merchant',barter:'Barter',rep:'Reputation',left:'left'};
  ...
  const selectedUpgrade=selectedBuilding ? buildingUpgradeInfo(selectedBuilding) : null;
  const reputationRank=useMemo(()=>readSettlementReputationRank(settlement),[settlement?.id,settlement?.name]);
  const market=useMemo(()=>getSettlementMarket(settlement,reputationRank),[settlement,reputationRank]);
  ...
  function marketLedgerEntry(type,label,caps,settlementCapsDelta){
    return {id:`market_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,day:Number(settlement.settlementDay||1),type,label,caps:Number(caps||0),settlementCapsDelta:Number(settlementCapsDelta||0),createdAt:Date.now()};
  }
  function tradeMaterial(kind,direction){
    if(onCommand || !setOwnerCharacter || !market.open || !market.prices?.[kind])return;
    const qty=Math.max(1,Math.floor(Number(tradeAmounts[kind]||1)));
    const unit=Number(market.prices[kind][direction]||0),total=qty*unit;
    const stock=normalizeStockpile(settlement.stockpile,settlement.resources?.materials);
    try{
      if(direction==="buy"){
        if(Number(stock.materials?.[kind]||0)<qty)throw new Error("MARKET_STOCK_LOW");
        const paid=debitPersonalResources(ownerCharacter,{caps:total,common:0,uncommon:0,rare:0});
        setOwnerCharacter(creditPersonalResources(paid.character,{caps:0,common:kind==="common"?qty:0,uncommon:kind==="uncommon"?qty:0,rare:kind==="rare"?qty:0}));
        onUpdate(current=>{const s=normalizeStockpile(current.stockpile,current.resources?.materials);const materials={...s.materials,[kind]:Math.max(0,Number(s.materials?.[kind]||0)-qty)};return {...current,stockpile:{...s,materials},resources:{...(current.resources||{}),caps:Number(current.resources?.caps||0)+total,materials:materials.common},marketLedger:[marketLedgerEntry("material_buy",(text[kind]||kind)+" ×"+qty,total,total),...(current.marketLedger||[])].slice(0,100)};});
      }else{
        if(Number(settlement.resources?.caps||0)<total)throw new Error("MARKET_CAPS_LOW");
        const paid=debitPersonalResources(ownerCharacter,{caps:0,common:kind==="common"?qty:0,uncommon:kind==="uncommon"?qty:0,rare:kind==="rare"?qty:0});
        setOwnerCharacter(creditPersonalResources(paid.character,{caps:total,common:0,uncommon:0,rare:0}));
        onUpdate(current=>{const s=normalizeStockpile(current.stockpile,current.resources?.materials);const materials={...s.materials,[kind]:Number(s.materials?.[kind]||0)+qty};return {...current,stockpile:{...s,materials},resources:{...(current.resources||{}),caps:Math.max(0,Number(current.resources?.caps||0)-total),materials:materials.common},marketLedger:[marketLedgerEntry("material_sell",(text[kind]||kind)+" ×"+qty,total,-total),...(current.marketLedger||[])].slice(0,100)};});
      }
      setNotice("");
    }catch(error){setNotice(error?.message||"TRADE_FAILED");}
  }
  function buyMarketItem(item){
    if(onCommand||!setOwnerCharacter||!market.open||Number(item.available||0)<1)return;
    const total=Math.max(1,Number(item.buyPrice||1));
    try{
      const paid=debitPersonalResources(ownerCharacter,{caps:total,common:0,uncommon:0,rare:0});
      setOwnerCharacter({...paid.character,inventoryItems:addMarketInventoryItem(paid.character.inventoryItems,item,1)});
      onUpdate(current=>({...current,
        marketPurchases:{...(current.marketPurchases||{}),[item.marketKey]:Math.max(0,Number(current.marketPurchases?.[item.marketKey]||0))+1},
        resources:{...(current.resources||{}),caps:Number(current.resources?.caps||0)+total},
        marketLedger:[marketLedgerEntry("item_buy",item.name,total,total),...(current.marketLedger||[])].slice(0,100)
      }));
      setNotice("");
    }catch(error){setNotice(error?.message||"TRADE_FAILED");}
  }
  function upgradeSelectedBuilding(){
    if(!selectedBuilding || !selectedUpgrade || selectedBuilding.upgrade || selectedBuilding.state!=="active")return;
    if(onCommand){void onCommand({type:"upgrade",buildingId:selectedBuilding.id});return;}
    const blockers=buildBlockers(settlement,ownerCharacter,selectedUpgrade.rule);
    if(blockers.length){setNotice(blockers.map(b=>b.label||b.key||b.kind).join(" · "));return;}
    onUpdate(current=>startUpgrade(current,ownerCharacter,undefined,selectedBuilding.id));
  }
  ...
  const navigation=mode=><button type="button" key={mode} className={panelMode===mode ? "is-active" : ""} aria-current={panelMode===mode ? "page" : undefined} onClick={()=>selectPanel(mode)}><SheetIcon name={{overview:'home',build:'plus',people:'people',resources:'bag',economy:'bag',defense:'shield',events:'notes',more:'more'}[mode]}/><span>{ui[mode] || text[mode]}</span></button>;
  return <div className={`pip-screen settlement-screen settlement-dashboard settlement-v2 ${onCommand ? 'is-shared-settlement' : ''} ${panelOpen ? 'is-panel-open' : ''}`}>
    ...
    <div className="settlement-layout settlement-dashboard-grid">
      <aside className="settlement-left-rail pip-panel"><nav className="settlement-left-nav">{['overview','build','people','resources','economy','defense','events'].map(navigation)}</nav><button type="button" className="pip-action-button settlement-exit" onClick={onBack}>← {text.back}</button></aside>
      ...
      <aside className="settlement-summary settlement-right-panel pip-panel" aria-label={ui[panelMode] || text[panelMode]}>
        ...
        {panelMode==="build" ? <>
          ...
          {selectedBuilding ? <div className="settlement-selected-card"><strong>{settlementBuildingName(selectedBuildingDef,language)}</strong><div className="settlement-balance"><span>{text.condition}</span><b>{Math.round(Number(selectedBuilding.condition ?? 100))}%</b></div>{selectedBuilding.state==="construction" ? <span>{text.progress}: {getConstructionProgress(selectedBuilding).progress}/{getConstructionProgress(selectedBuilding).required} d</span> : <span>{text.active}</span>}{selectedBuilding.state==="active" && selectedBuildingProduction(settlement,selectedBuilding,language) ? <strong className="settlement-production-forecast">{selectedBuildingProduction(settlement,selectedBuilding,language)}</strong> : null}
            ...
            {selectedUpgrade && selectedBuilding.state==='active' && !selectedBuilding.upgrade ? <section className="settlement-upgrade-card"><div><strong>{tradeText.level} {selectedUpgrade.level} → {tradeText.next} {selectedUpgrade.nextLevel}</strong><small>{settlementBuildingName(SETTLEMENT_BUILDINGS[selectedUpgrade.targetType],language)}</small></div><small>{tradeText.upgradeCost}: {formatRulebookCost(selectedUpgrade.rule)} · {tradeText.upgradeTime}: {selectedUpgrade.rule.constructionDays}d</small>{selectedUpgrade.changes.length?<div className="settlement-upgrade-changes"><b>{tradeText.changes}</b>{selectedUpgrade.changes.map(change=><span key={change.key}>{change.key}: {change.from} → {change.to}</span>)}</div>:null}<button type="button" className="pip-action-button settlement-primary" disabled={!canEdit || Boolean(selectedBuilding.upgrade) || (payment ? !payment.canAfford({type:'upgradePersonal',buildingId:selectedBuilding.id}) : false)} onClick={upgradeSelectedBuilding}>{tradeText.upgrade}</button></section> : null}
        ...
        {panelMode==="resources" ? <div className="settlement-resource-panel"><div className="pip-panel-title">{text.resources}</div><button type="button" className="pip-action-button settlement-open-economy" onClick={()=>selectPanel("economy")}>{ui.economy}</button><SettlementDailySummary settlement={settlement} language={language} canEdit={canEdit} onSupply={resource=>{if(onCommand)void onCommand({type:"supplies",resource});else onUpdate(current=>reserveProvisions(current,resource));}}/><div className="settlement-balance"><span>{text.food}</span><b>{attributes.food}</b></div><div className="settlement-balance"><span>{text.water}</span><b>{attributes.water}</b></div><div className="settlement-balance"><span>{text.power}</span><b>{attributes.power}</b></div><div className="settlement-balance"><span>{text.beds}</span><b>{attributes.beds}</b></div><div className="settlement-balance"><span>{text.common}</span><b>{Math.floor(stockpile.materials.common)}</b></div><div className="settlement-balance"><span>{text.uncommon}</span><b>{Math.floor(stockpile.materials.uncommon)}</b></div><div className="settlement-balance"><span>{text.rare}</span><b>{Math.floor(stockpile.materials.rare)}</b></div><small>{Math.floor(snapshot.stockpileCapacityLbs)} lbs</small><section className="settlement-market-card"><div className="pip-panel-title">{tradeText.title}</div>{market.open?<><small>{tradeText.tier}: {market.tier} · {tradeText.merchant}: {market.merchant.workerName||"—"} · {tradeText.barter}: {market.merchant.barter} · {tradeText.rep}: {market.reputationRank}/5</small>{Object.entries(market.prices).map(([kind,price])=>{const qty=Math.max(1,Number(tradeAmounts[kind]||1));const stockQty=Math.floor(Number(stockpile.materials?.[kind]||0));let playerQty=0;try{playerQty=Number(checkedPlayerResources(ownerCharacter)?.[kind]||0);}catch{}return <div className="settlement-market-row" key={kind}><div><strong>{text[kind]||kind}</strong><small>{tradeText.buyPrice}: {price.buy} · {tradeText.sellPrice}: {price.sell} {tradeText.caps} · stock {stockQty} · player {playerQty}</small></div><input className="pip-input" type="number" min="1" value={qty} onChange={e=>setTradeAmounts(current=>({...current,[kind]:Math.max(1,Number(e.target.value)||1)}))}/><button type="button" className="pip-action-button" disabled={Boolean(onCommand)||!setOwnerCharacter||stockQty<qty||Number(ownerCharacter?.caps||0)<qty*price.buy} onClick={()=>tradeMaterial(kind,"buy")}>{tradeText.buy} · {qty*price.buy}</button><button type="button" className="pip-action-button" disabled={Boolean(onCommand)||!setOwnerCharacter||playerQty<qty||Number(settlement.resources?.caps||0)<qty*price.sell} onClick={()=>tradeMaterial(kind,"sell")}>{tradeText.sell} · {qty*price.sell}</button></div>})}<div className="settlement-market-daily"><strong>{tradeText.daily} · Day {market.day}</strong>{market.inventory.map(item=><div className="settlement-market-item" key={item.marketKey}><div><b>{item.name}</b><small>{item.category} · rarity {item.rarity||0} · {item.available} {tradeText.left}</small></div><span>{item.buyPrice} {tradeText.caps}</span><button type="button" className="pip-action-button" disabled={Boolean(onCommand)||!setOwnerCharacter||item.available<1||Number(ownerCharacter?.caps||0)<item.buyPrice} onClick={()=>buyMarketItem(item)}>{tradeText.buy}</button></div>)}</div></>:<small>{tradeText.closed}</small>}</section><section className="settlement-resource-transfer"><div className="pip-panel-title">{text.transfer}</div><small>{text.produced}</small>{["caps","food","water","common","uncommon","rare"].map(kind=>{const playerHave=kind==="caps"?Number(ownerCharacter?.caps||0):kind==="food"||kind==="water"?playerSupplyCount(kind):(()=>{try{return checkedPlayerResources(ownerCharacter)[kind]||0;}catch{return 0;}})();const settlementHave=kind==="caps"?Number(settlement.resources?.caps||0):kind==="food"||kind==="water"?Number(stockpile.provisions?.[kind]||0):Number(stockpile.materials?.[kind]||0);return <div className="settlement-transfer-row" key={kind}><div><strong>{text[kind]||kind}</strong><small>{text.playerHave}: {playerHave} · {text.settlementHave}: {settlementHave}</small></div><input className="pip-input" type="number" min="1" value={transferAmounts[kind]} onChange={e=>setTransferAmounts(current=>({...current,[kind]:Math.max(1,Number(e.target.value)||1)}))}/><button type="button" className="pip-action-button" disabled={!setOwnerCharacter||Boolean(onCommand)} onClick={()=>transferLocalResource(kind,"deposit")}>→ {text.deposit}</button><button type="button" className="pip-action-button" disabled={!setOwnerCharacter||Boolean(onCommand)} onClick={()=>transferLocalResource(kind,"withdraw")}>← {text.withdraw}</button></div>})}</section></div> : null}
        {panelMode==="economy" ? <SettlementEconomyPanel settlement={settlement} language={language} market={market}/> : null}
