...
export default function SettlementScreen({ settlement, onUpdate, onBack, onCommand, canEdit=true, sharedControls, payment }) {
  ...
  function saveDefensePlan(){if(!canEdit || !activeAttack)return;if(onCommand){void onCommand({type:"attackPlan",attackId:activeAttack.id,defenderIds:defenseDraft});return;}onUpdate(current=>setSettlementDefensePlan(current,activeAttack.id,defenseDraft));}
  async function playRaidBattle(){
    if(!canEdit || !activeAttack || battleBusy)return;
    setBattleBusy(true);setBattleError("");
    try{
      if(onCommand){
        const planned=await onCommand({type:"attackPlan",attackId:activeAttack.id,defenderIds:defenseDraft});
        if(!planned)throw new Error("DEFENSE_PLAN_SAVE_FAILED");
      }
      const prepared=await startSettlementRaidBattle(liveSession,settlement,{...activeAttack,defenderIds:defenseDraft},defenseDraft);
  ...
  return <div className={`pip-screen settlement-screen settlement-dashboard settlement-v2 ${onCommand ? 'is-shared-settlement' : ''} ${panelOpen ? 'is-panel-open' : ''}`}>
    ...
    <div className="settlement-layout settlement-dashboard-grid">
      ...
      <aside className="settlement-summary settlement-right-panel pip-panel" aria-label={ui[panelMode] || text[panelMode]}>
        ...
        {panelMode==="defense" ? <div className={`settlement-defense-panel ${activeAttack ? "is-alert" : ""}`}><div className="pip-panel-title">{text.attack}</div><div className="settlement-balance"><span>{text.defense}</span><b>{attributes.defense}{activeAttack ? ` + ${defenseDraft.length}` : ""}</b></div><div className="settlement-balance"><span>{text.riskCheck}</span><b>{attackDice ? `${attackDice}d20` : "—"}</b></div>{activeAttack ? <><strong>{activeAttack.state==="warning" ? text.warning : text.attackActive}</strong><span>{activeAttack.faction?.replaceAll("_"," ")}</span><div className="settlement-balance"><span>{text.strength}</span><b>{activeAttack.strength}</b></div><span>{text.startsIn}: {formatBuildTime(Number(activeAttack.startsAt)-Date.now())}</span><div className="pip-panel-title">{text.defenders} · {defenseDraft.length}</div>{(settlement.settlers || []).map(settler=><label className="settlement-person" key={settler.id}><span><strong>{settler.name}</strong><small>{settler.status || "idle"}</small></span><input type="checkbox" disabled={!canEdit || Number(settler.health ?? 100)<=1} checked={defenseDraft.includes(settler.id)} onChange={()=>toggleDefender(settler.id)}/></label>)}<button type="button" className="pip-action-button" disabled={!canEdit} onClick={saveDefensePlan}>{text.saveDefense}</button>{liveSession?.mode==="host" && liveSession?.status==="online" ? <button type="button" className="pip-action-button settlement-primary" disabled={!canEdit || battleBusy} onClick={playRaidBattle}>{battleBusy ? text.battlePreparing : activeAttack.tacticalSceneId ? text.resumeBattle : text.playBattle}</button> : <small>{text.gmSessionRequired}</small>}{battleError ? <small role="alert">{battleError}</small> : null}<button type="button" className="pip-action-button" disabled={!canEdit || battleBusy} onClick={resolveAttackNow}>{text.autoDefense}</button></> : <span>{lastResolvedAttack ? `${lastResolvedAttack.result==="victory" ? text.victory : text.defeat} · ${lastResolvedAttack.faction?.replaceAll("_"," ")}` : text.noThreat}</span>}</div> : null}
