  ...
  function tradeMaterial(kind,direction){
    ...
    try{
      if(direction==="buy"){
        ...
        onUpdate(current=>{const s=normalizeStockpile(current.stockpile,current.resources?.materials);const materials={...s.materials,[kind]:Math.max(0,Number(s.materials?.[kind]||0)-qty)};return {...current,stockpile:{...s,materials},resources:{...(current.resources||{}),caps:Number(current.resources?.caps||0)+total,materials:materials.common}};});
      }else{
        ...
        onUpdate(current=>{const s=normalizeStockpile(current.stockpile,current.resources?.materials);const materials={...s.materials,[kind]:Number(s.materials?.[kind]||0)+qty};return {...current,stockpile:{...s,materials},resources:{...(current.resources||{}),caps:Math.max(0,Number(current.resources?.caps||0)-total),materials:materials.common}};});
      }
      setNotice("");
