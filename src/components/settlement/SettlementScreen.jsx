...
import { SETTLER_PERKS, SETTLER_SKILLS, advanceSettlerProfile, settlerProfileLabel, settlerXpForNextLevel } from "../../utils/settlementSettlerProfile.js";
...
export default function SettlementScreen({ settlement, onUpdate, onBack, onCommand, canEdit=true, sharedControls, payment, canClaimProfit=false, onRemoveGuestNpc, ownerCharacter=null, setOwnerCharacter=null }) {
  ...
  const recruitment=useMemo(()=>getSettlementRecruitmentStatus(settlement),[settlement]);
  ...
  const recruitmentBlockedText = recruitment.blockedReason==="NO_POWERED_BEACON"
    ? text.noBeacon
    : recruitment.blockedReason==="NO_CAPACITY"
    ? text.noCapacity
    : recruitment.blockedReason==="NO_BEDS"
    ? text.noBeds
    : recruitment.blockedReason==="NO_FOOD"
    ? text.noFood
    : recruitment.blockedReason==="NO_WATER"
    ? text.noWater
    : "";
  ...
  return <div className={`pip-screen settlement-screen settlement-dashboard settlement-v2 ${onCommand ? 'is-shared-settlement' : ''} ${panelOpen ? 'is-panel-open' : ''}`}>
    ...
    <div className="settlement-layout settlement-dashboard-grid">
      ...
      <main className="settlement-map-wrap settlement-center-panel">
        ...
        <section className={`settlement-recruitment-card pip-panel ${recruitment.chancePercent>0?"is-active":"is-blocked"}`}>
          <div>
            <small>{text.recruitmentChance}</small>
            <strong>{recruitment.chancePercent}%</strong>
            <span>{text.recruitmentDaily}</span>
          </div>
          <div className="settlement-recruitment-card__meta">
            <span>♥ {recruitment.target}/20</span>
            <span>👥 {recruitment.people}/{recruitment.capacity}</span>
          </div>
          {recruitment.chancePercent<=0?<small>{text.recruitmentBlocked}: {recruitmentBlockedText}</small>:null}
        </section>
      ...
      <aside className="settlement-summary settlement-right-panel pip-panel" aria-label={ui[panelMode] || text[panelMode]}>
        ...
        {panelMode==="build" ? <>
          ...
          {selectedBuilding ? <div className="settlement-selected-card"><strong>{settlementBuildingName(selectedBuildingDef,language)}</strong>${selectedBuilding.type===\"radio_beacon\" ? <div className=\"settlement-recruitment-inline\"><span>{text.recruitmentChance}</span><b>{recruitment.chancePercent}%</b><small>{recruitment.chancePercent>0 ? text.recruitmentDaily : recruitmentBlockedText}</small></div> : null}<div className="settlement-balance"><span>{text.condition}</span><b>{Math.round(Number(selectedBuilding.condition ?? 100))}%</b></div>{selectedBuilding.state==="construction" ? <span>{text.progress}: {getConstructionProgress(selectedBuilding).progress}/{getConstructionProgress(selectedBuilding).required} d</span> : <span>{text.active}</span>}
