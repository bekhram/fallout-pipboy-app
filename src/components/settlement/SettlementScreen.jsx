...
import { constructionRequirementBlockers, upgradeRule } from '../../utils/settlementDevelopment.js';
...
import { butcherSettlementBrahmin, reserveCropFertilizer } from "../../utils/settlementResources.js";
...
import { buySettlementSupply, reservePurchasedSupply, sellSettlementSurplus, setCaravanTravelDays } from "../../utils/settlementTrade.js";
import { nextStoreSpecialty, setStoreSpecialty, STORE_SPECIALTIES, storeSpecialty, storeTradeProfile } from "../../utils/settlementStores.js";
import { CROP_TYPES, cropCapacity, plantSettlementCrop, plantedCrops, removeSettlementCrop } from "../../utils/settlementCrops.js";
import { contractorConstructionRule, settlementPerkRank } from "../../utils/settlementLeaderRules.js";
import { knowsSettlementRecipe, setSettlementRecipeKnown } from "../../utils/settlementRecipes.js";
import { useLiveSessionBridge } from "../../utils/liveSessionBridge.js";
import { canAffordRoom, canAffordRulebookBuilding, canAffordSettlementRule, createConstructionBuilding, createRoomConstruction, getConstructionProgress, getRoomConstructionProgress, getSettlementRulebookSnapshot, getStructureRoomCapacity, normalizeStockpile, payRoomCost, payRulebookBuildingCost, paySettlementRuleCost } from "../../utils/settlementDayEngine.js";
...
export default function SettlementScreen({ settlement, character=null, onCharacterChange=null, onUpdate, onBack, onCommand, canEdit=true, sharedControls, payment, canClaimProfit=false }) {
  ...
  const [contractorMode,setContractorMode]=useState("normal");
  ...
  const contractorRank=settlementPerkRank(character,"Contractor");
  const effectiveSelectedRule=selectedRule ? contractorConstructionRule(selectedRule,character,contractorMode) : null;
  const rawSelectedRequirementBlockers=selectedRule ? constructionRequirementBlockers(character,selectedRule,selectedType) : [];
  const contractorSkillBypass=contractorRank>=2 && rawSelectedRequirementBlockers.some(blocker=>blocker.kind==="skill");
  const selectedRequirementBlockers=rawSelectedRequirementBlockers.filter(blocker=>!(contractorRank>=2 && blocker.kind==="skill"));
  const recipeText=({
    en:{recipe:"Rare recipe",known:"RECIPE LEARNED",unknown:"RECIPE NOT LEARNED",learn:"MARK AS LEARNED",forget:"REMOVE RECIPE"},
    ru:{recipe:"Редкий рецепт",known:"РЕЦЕПТ ИЗУЧЕН",unknown:"РЕЦЕПТ НЕ ИЗУЧЕН",learn:"ОТМЕТИТЬ КАК ИЗУЧЕННЫЙ",forget:"УБРАТЬ РЕЦЕПТ"},
    uk:{recipe:"Рідкісний рецепт",known:"РЕЦЕПТ ВИВЧЕНО",unknown:"РЕЦЕПТ НЕ ВИВЧЕНО",learn:"ПОЗНАЧИТИ ЯК ВИВЧЕНИЙ",forget:"ПРИБРАТИ РЕЦЕПТ"},
    pl:{recipe:"Rzadki przepis",known:"PRZEPIS POZNANY",unknown:"PRZEPIS NIEPOZNANY",learn:"OZNACZ JAKO POZNANY",forget:"USUŃ PRZEPIS"}
  })[language] || {recipe:"Rare recipe",known:"RECIPE LEARNED",unknown:"RECIPE NOT LEARNED",learn:"MARK AS LEARNED",forget:"REMOVE RECIPE"};
  const contractorText=({
    en:{title:"Contractor",normal:"Normal",cheap:"Half materials · −2 Happiness",careful:"Half materials · ×2 time"},
    ru:{title:"Подрядчик",normal:"Обычно",cheap:"Половина материалов · −2 счастья",careful:"Половина материалов · ×2 время"},
    uk:{title:"Підрядник",normal:"Звичайно",cheap:"Половина матеріалів · −2 щастя",careful:"Половина матеріалів · ×2 час"},
    pl:{title:"Wykonawca",normal:"Normalnie",cheap:"Połowa materiałów · −2 Szczęścia",careful:"Połowa materiałów · ×2 czasu"}
  })[language] || {title:"Contractor",normal:"Normal",cheap:"Half materials · −2 Happiness",careful:"Half materials · ×2 time"};
  ...
  const storeText=({
    en:{specialty:"Store type",general:"General",armor:"Armor",weapons:"Weapons",food_drink:"Food & Drink",clothing:"Clothing",medical:"Medical",rarity:"Standard rarity",cash:"Vendor caps",checks:"Rare checks / week"},
    ru:{specialty:"Тип магазина",general:"Обычный",armor:"Броня",weapons:"Оружие",food_drink:"Еда и напитки",clothing:"Одежда",medical:"Медицина",rarity:"Обычная редкость",cash:"Крышки торговца",checks:"Проверок редкости / неделю"},
    uk:{specialty:"Тип магазину",general:"Звичайний",armor:"Броня",weapons:"Зброя",food_drink:"Їжа та напої",clothing:"Одяг",medical:"Медицина",rarity:"Стандартна рідкість",cash:"Кришки торговця",checks:"Перевірок рідкості / тиждень"},
    pl:{specialty:"Typ sklepu",general:"Ogólny",armor:"Pancerze",weapons:"Broń",food_drink:"Jedzenie i napoje",clothing:"Odzież",medical:"Medyczny",rarity:"Standardowa rzadkość",cash:"Kapsle kupca",checks:"Testy rzadkości / tydzień"}
  })[language] || {specialty:"Store type",general:"General",armor:"Armor",weapons:"Weapons",food_drink:"Food & Drink",clothing:"Clothing",medical:"Medical",rarity:"Standard rarity",cash:"Vendor caps",checks:"Rare checks / week"};
  const tradeText=({
    en:{title:"TRADE",trader:"Trader available",noTrader:"No trader in settlement",sellFood:"SELL 1 FOOD",sellWater:"SELL 1 WATER",buyFood:"BUY FOOD · 1 INCOME",buyWater:"BUY WATER · 1 INCOME",purchased:"Purchased reserve",use:"USE +1 NEXT DAY",travel:"Caravan travel time",days:"days",trip:"Caravan away until day"},
    ru:{title:"ТОРГОВЛЯ",trader:"Торговец в поселении",noTrader:"Сейчас торговца нет",sellFood:"ПРОДАТЬ 1 ЕДУ",sellWater:"ПРОДАТЬ 1 ВОДУ",buyFood:"КУПИТЬ ЕДУ · 1 ДОХОД",buyWater:"КУПИТЬ ВОДУ · 1 ДОХОД",purchased:"Купленный запас",use:"ИСПОЛЬЗОВАТЬ +1 НА СЛЕД. ДЕНЬ",travel:"Срок торгового каравана",days:"дней",trip:"Караван вернётся в день"},
    uk:{title:"ТОРГІВЛЯ",trader:"Торговець у поселенні",noTrader:"Зараз торговця немає",sellFood:"ПРОДАТИ 1 ЇЖУ",sellWater:"ПРОДАТИ 1 ВОДУ",buyFood:"КУПИТИ ЇЖУ · 1 ДОХІД",buyWater:"КУПИТИ ВОДУ · 1 ДОХІД",purchased:"Куплений запас",use:"ВИКОРИСТАТИ +1 НА НАСТ. ДЕНЬ",travel:"Тривалість торгового каравану",days:"днів",trip:"Караван повернеться у день"},
    pl:{title:"HANDEL",trader:"Handlarz w osadzie",noTrader:"Brak handlarza",sellFood:"SPRZEDAJ 1 ŻYWNOŚĆ",sellWater:"SPRZEDAJ 1 WODĘ",buyFood:"KUP ŻYWNOŚĆ · 1 DOCHÓD",buyWater:"KUP WODĘ · 1 DOCHÓD",purchased:"Kupiony zapas",use:"UŻYJ +1 NASTĘPNEGO DNIA",travel:"Czas karawany",days:"dni",trip:"Karawana wróci w dniu"}
  })[language] || {title:"TRADE",trader:"Trader available",noTrader:"No trader in settlement",sellFood:"SELL 1 FOOD",sellWater:"SELL 1 WATER",buyFood:"BUY FOOD · 1 INCOME",buyWater:"BUY WATER · 1 INCOME",purchased:"Purchased reserve",use:"USE +1 NEXT DAY",travel:"Caravan travel time",days:"days",trip:"Caravan away until day"};
  const brahminText=({
    en:{title:"BRAHMIN",milk:"Brahmin milk",meat:"Brahmin meat",fertilizer:"Fertilizer",butcherFood:"BUTCHER · +2 FOOD NEXT DAY",butcherMeat:"BUTCHER · 2 MEAT",needsFarmer:"Requires Tend Crops action"},
    ru:{title:"БРАМИНЫ",milk:"Молоко брамина",meat:"Мясо брамина",fertilizer:"Удобрение",butcherFood:"ЗАБИТЬ · +2 ЕДЫ НА СЛЕД. ДЕНЬ",butcherMeat:"ЗАБИТЬ · 2 МЯСА",needsFarmer:"Требуется действие Уход за культурами"},
    uk:{title:"БРАМІНИ",milk:"Молоко браміна",meat:"М’ясо браміна",fertilizer:"Добриво",butcherFood:"ЗАБИТИ · +2 ЇЖІ НА НАСТ. ДЕНЬ",butcherMeat:"ЗАБИТИ · 2 М’ЯСА",needsFarmer:"Потрібна дія Догляд за культурами"},
    pl:{title:"BRAMINY",milk:"Mleko bramina",meat:"Mięso bramina",fertilizer:"Nawóz",butcherFood:"UBÓJ · +2 ŻYWNOŚĆ NASTĘPNEGO DNIA",butcherMeat:"UBÓJ · 2 MIĘSA",needsFarmer:"Wymaga akcji Uprawa"}
  })[language] || {title:"BRAHMIN",milk:"Brahmin milk",meat:"Brahmin meat",fertilizer:"Fertilizer",butcherFood:"BUTCHER · +2 FOOD NEXT DAY",butcherMeat:"BUTCHER · 2 MEAT",needsFarmer:"Requires Tend Crops action"};
  const cropText=({
    en:{title:"CROPS",plant:"PLANT",remove:"REMOVE",stock:"Food items available",full:"Plot is full"},
    ru:{title:"КУЛЬТУРЫ",plant:"ПОСАДИТЬ",remove:"УБРАТЬ",stock:"Доступно пищевых предметов",full:"Грядка заполнена"},
    uk:{title:"КУЛЬТУРИ",plant:"ПОСАДИТИ",remove:"ПРИБРАТИ",stock:"Доступно харчових предметів",full:"Грядка заповнена"},
    pl:{title:"UPRAWY",plant:"POSADŹ",remove:"USUŃ",stock:"Dostępne przedmioty żywności",full:"Grządka pełna"}
  })[language] || {title:"CROPS",plant:"PLANT",remove:"REMOVE",stock:"Food items available",full:"Plot is full"};
  ...
  const enoughResources=selectedType ? payment ? payment.canAfford({type:"buildPersonal",buildingType:selectedType}) : canAffordSettlementRule(settlement,effectiveSelectedRule || selectedRule) : true;
  const requirementsMet=!selectedType || selectedRequirementBlockers.length===0;
  ...
  async function createBuildingAt(x,y) {
    ...
    if(selectedRequirementBlockers.length){setNotice(selectedRequirementBlockers.map(blocker=>blocker.label || blocker.kind).join(" · "));return;}
    const family=selectedRule.effects?.storeFamily;
    const specialty=family==="trading" ? nextStoreSpecialty(settlement) : family==="medical" ? "medical" : "";
    if(family==="trading" && !specialty){setNotice("STORE_TYPE_EXISTS");return;}
    if(family==="medical" && (settlement.buildings || []).some(building=>building.state!=="destroyed" && getRulebookBuilding(building.type)?.effects?.storeFamily==="medical")){setNotice("STORE_TYPE_EXISTS");return;}
    if(onCommand){if(await onCommand({type:"build",buildingType:selectedDef.id,x,y,...(specialty?{storeSpecialty:specialty}:{})})){setSelectedType(null);setHoverCell(null);setNotice("");}return;}
    const now=Date.now();onUpdate(current=>{
      const rule=effectiveSelectedRule || selectedRule;
      let paid=paySettlementRuleCost(current,rule);
      const penalty=Math.max(0,Number(rule?.happinessPenalty || 0))+(contractorSkillBypass?1:0);
      if(penalty){
        const happiness=Math.max(1,Number(paid.attributes?.happiness || 10)-penalty);
        paid={...paid,attributes:{...(paid.attributes||{}),happiness},resources:{...(paid.resources||{}),happiness}};
      }
      const building=createConstructionBuilding({id:`building_${now}_${Math.random().toString(36).slice(2,7)}`,type:selectedDef.id,x,y,now,ruleOverride:rule});
      return {...paid,buildings:[...(paid.buildings || []),{...building,...(specialty?{storeSpecialty:specialty}:{}),contractorSkillBypass}]};
    });
  ...
  function addRoom(type) {
    ...
    const baseRoomRule=ROOMS[type],effectiveRoomRule=contractorConstructionRule(baseRoomRule,character,contractorMode);
    if((payment ? !payment.canAfford({type:"roomPersonal",buildingId:selectedBuilding?.id,roomType:type}) : !canAffordSettlementRule(settlement,effectiveRoomRule))){setNotice(payment ? personalText.errors.PERSONAL_RESOURCES_INSUFFICIENT : text.insufficient);return;}
    const rawRoomBlockers=constructionRequirementBlockers(character,baseRoomRule);
    const roomSkillBypass=contractorRank>=2 && rawRoomBlockers.some(blocker=>blocker.kind==="skill");
    const roomBlockers=rawRoomBlockers.filter(blocker=>!(contractorRank>=2 && blocker.kind==="skill"));
    if(roomBlockers.length){setNotice(roomBlockers.map(blocker=>blocker.label || blocker.kind).join(" · "));return;}
    const room=createRoomConstruction(type,Date.now(),effectiveRoomRule);if(!room)return;
    onUpdate(current=>{
      let paid=paySettlementRuleCost(current,effectiveRoomRule);
      const penalty=Math.max(0,Number(effectiveRoomRule?.happinessPenalty || 0))+(roomSkillBypass?1:0);
      if(penalty){const happiness=Math.max(1,Number(paid.attributes?.happiness || 10)-penalty);paid={...paid,attributes:{...(paid.attributes||{}),happiness},resources:{...(paid.resources||{}),happiness}};}
      return {...paid,buildings:(paid.buildings || []).map(building=>building.id===selectedBuilding.id ? {...building,rooms:[...(building.rooms || []),{...room,contractorSkillBypass:roomSkillBypass}]} : building)};
    });setNotice("");
  }
  ...
  function tradeSell(resource){
    if(!canEdit || onCommand)return;
    try{onUpdate(current=>sellSettlementSurplus(current,resource,1));setNotice("");}
    catch(error){setNotice(error?.message || "TRADE");}
  }
  function tradeBuy(resource){
    if(!canEdit || onCommand)return;
    try{onUpdate(current=>buySettlementSupply(current,resource,1));setNotice("");}
    catch(error){setNotice(error?.message || "TRADE");}
  }
  function usePurchasedSupply(resource){
    if(!canEdit || onCommand)return;
    try{onUpdate(current=>reservePurchasedSupply(current,resource,1));setNotice("");}
    catch(error){setNotice(error?.message || "TRADE");}
  }
  function setTradeTravelDays(value){
    if(!canEdit || onCommand)return;
    onUpdate(current=>setCaravanTravelDays(current,value));
  }
  function changeStoreSpecialty(value){
    if(!canEdit || onCommand || !selectedBuilding)return;
    try{onUpdate(current=>setStoreSpecialty(current,selectedBuilding.id,value));setNotice("");}
    catch(error){setNotice(error?.message || "STORE_TYPE_EXISTS");}
  }
  function plantCrop(type){
    if(!canEdit || onCommand || !selectedBuilding)return;
    const blockers=constructionRequirementBlockers(character,{skill:{name:"Survival",rank:1},rarity:"common"});
    if(blockers.length){setNotice(blockers.map(blocker=>blocker.label||blocker.kind).join(" · "));return;}
    try{onUpdate(current=>plantSettlementCrop(current,selectedBuilding.id,type));setNotice("");}
    catch(error){setNotice(error?.message || "CROP");}
  }
  function removeCrop(cropId){
    if(!canEdit || onCommand || !selectedBuilding)return;
    try{onUpdate(current=>removeSettlementCrop(current,selectedBuilding.id,cropId));setNotice("");}
    catch(error){setNotice(error?.message || "CROP");}
  }
  function butcherBrahmin(mode){
    if(!canEdit || onCommand)return;
    try{onUpdate(current=>butcherSettlementBrahmin(current,mode));setNotice("");}
    catch(error){setNotice(error?.message || "BRAHMIN");}
  }
  function toggleSelectedRecipe(){
    if(!selectedType || !onCharacterChange || !character)return;
    const known=knowsSettlementRecipe(character,selectedType);
    onCharacterChange(current=>setSettlementRecipeKnown(current,selectedType,!known));
    setNotice(!known ? recipeText.known : recipeText.unknown);
  }
  ...
  return <div className={`pip-screen settlement-screen settlement-dashboard settlement-v2 ${onCommand ? 'is-shared-settlement' : ''} ${settlement.offlineStandalone ? 'is-offline-settlement' : ''} ${panelOpen ? 'is-panel-open' : ''} ${editorMode ? 'is-editor-mode' : ''}`}>
    ...
    <div className="settlement-layout settlement-dashboard-grid">
      ...
      <aside className="settlement-summary settlement-right-panel pip-panel" aria-label={ui[panelMode] || text[panelMode]}>
        ...
        {panelMode==="build" && !selectedBuilding ? <><div className="settlement-build-categories" role="tablist" aria-label={text.build}>{BUILD_CATEGORIES.map(category=><button key={category} type="button" className={selectedCategory===category ? "is-selected" : ""} onClick={()=>{setSelectedCategory(category);setSelectedType(null);setHoverCell(null);setNotice("");}}>{categoryLabels[category]}</button>)}</div><div className="settlement-build-menu">{visibleBuildings.map(def=>{const asset=getSettlementAsset(def.asset);const rule=getRulebookBuilding(def.id);const rawBlockers=constructionRequirementBlockers(character,rule,def.id);const blockers=rawBlockers.filter(blocker=>!(contractorRank>=2 && blocker.kind==="skill"));const hardBlockers=blockers.filter(blocker=>blocker.kind!=="recipe");const family=rule?.effects?.storeFamily;const duplicateStore=family==="medical" ? (settlement.buildings || []).some(building=>building.state!=="destroyed" && getRulebookBuilding(building.type)?.effects?.storeFamily==="medical") : family==="trading" ? !nextStoreSpecialty(settlement) : false;return <button key={def.id} type="button" disabled={!canEdit || hardBlockers.length>0 || duplicateStore} className={selectedType===def.id ? "is-selected" : ""} onClick={()=>{setSelectedType(def.id);setStoredBuildingId(null);setSelectedBuildingId(null);setMovingBuildingId(null);setNotice("");setPanelOpen(false);}}><div className="settlement-build-menu__preview">{asset ? <img src={asset} alt=""/> : <span>{BUILDING_ICONS[def.id] || "⌂"}</span>}</div><span>{settlementBuildingName(def,language)}</span><small>{def.footprint.width}×{def.footprint.height} · {formatRulebookCost(rule)} · {rule.constructionDays}d{blockers.length ? ` · ⚠ ${blockers.map(blocker=>blocker.label || blocker.kind).join(", ")}` : duplicateStore ? " · ⚠ STORE" : ""}</small></button>;})}</div></> : null}
        {panelMode==="build" ? <>
          ...
          {selectedDef && selectedRule ? <div className="settlement-selected-card"><strong>{settlementBuildingName(selectedDef,language)}</strong><span>{selectedDef.footprint.width}×{selectedDef.footprint.height}</span>{contractorRank>=1 && !onCommand ? <label>{contractorText.title}<select className="pip-input" value={contractorMode} onChange={event=>setContractorMode(event.target.value)}><option value="normal">{contractorText.normal}</option><option value="cheap">{contractorText.cheap}</option><option value="careful">{contractorText.careful}</option></select></label> : null}<span>{formatRulebookCost(effectiveSelectedRule || selectedRule)}</span><span>{text.construction}: {effectiveSelectedRule?.constructionDays || selectedRule.constructionDays} d</span>{String(selectedRule.rarity||"").toLowerCase()==="rare" && <div className="settlement-recipe-status"><strong>{knowsSettlementRecipe(character,selectedType) ? recipeText.known : recipeText.unknown}</strong><button type="button" className="pip-action-button" disabled={!onCharacterChange} onClick={toggleSelectedRecipe}>{knowsSettlementRecipe(character,selectedType) ? recipeText.forget : recipeText.learn}</button></div>}{contractorSkillBypass && <small>Contractor 2 · −1 Happiness</small>}<button type="button" className="pip-action-button" onClick={()=>{setSelectedType(null);setHoverCell(null);setContractorMode("normal");}}>{text.cancel}</button></div> : null}
          {selectedBuilding ? <div className="settlement-selected-card"><strong>{settlementBuildingName(selectedBuildingDef,language)}</strong><div className="settlement-balance"><span>{text.condition}</span><b>{Math.round(Number(selectedBuilding.condition ?? 100))}%</b></div>{selectedBuilding.state==="construction" ? <span>{text.progress}: {getConstructionProgress(selectedBuilding).progress}/{getConstructionProgress(selectedBuilding).required} d</span> : <span>{text.active}</span>}
            ...
            {selectedBuildingRule?.effects?.store ? (()=>{const profile=storeTradeProfile(settlement,selectedBuilding);return <section className="settlement-store-profile"><label>{storeText.specialty}{selectedBuildingRule.effects.storeFamily==="trading" && !onCommand ? <select className="pip-input" value={storeSpecialty(selectedBuilding)} onChange={event=>changeStoreSpecialty(event.target.value)}>{STORE_SPECIALTIES.map(type=><option key={type} value={type} disabled={(settlement.buildings || []).some(other=>other.id!==selectedBuilding.id && other.state!=="destroyed" && storeSpecialty(other)===type)}>{storeText[type]}</option>)}</select> : <strong>{storeText[profile?.specialty] || profile?.specialty}</strong>}</label>{profile && <><div className="settlement-balance"><span>{storeText.rarity}</span><b>{profile.standardRarity}</b></div><div className="settlement-balance"><span>{storeText.cash}</span><b>{profile.capsBase}+{profile.capsDice}d20</b></div><div className="settlement-balance"><span>{storeText.checks}</span><b>{profile.rareChecks}</b></div></>}</section>;})() : null}
            {selectedBuildingRule?.effects?.cropSlots ? <section className="settlement-crop-panel"><div className="pip-panel-title">{cropText.title} · {plantedCrops(selectedBuilding).length}/{cropCapacity(selectedBuilding)}</div><small>{cropText.stock}: {Math.floor(Number(stockpile.provisions?.food || 0))}</small><div className="settlement-crop-list">{plantedCrops(selectedBuilding).map(crop=><div className="settlement-crop-row" key={crop.id}><span>{crop.type}{crop.state==="construction" ? " · 1d" : ""}</span><button type="button" className="pip-action-button" disabled={!canEdit || Boolean(onCommand)} onClick={()=>removeCrop(crop.id)}>×</button></div>)}</div>{plantedCrops(selectedBuilding).length<cropCapacity(selectedBuilding) ? <div className="settlement-crop-buttons">{CROP_TYPES.map(type=><button type="button" className="pip-action-button" key={type} disabled={!canEdit || Boolean(onCommand) || Number(stockpile.provisions?.food || 0)<1} onClick={()=>plantCrop(type)}>{cropText.plant} {type}</button>)}</div> : <small>{cropText.full}</small>}</section> : null}
        ...
        {panelMode==="resources" ? <div className="settlement-resource-panel"><div className="pip-panel-title">{text.resources}</div><SettlementDailySummary settlement={settlement} language={language} canEdit={canEdit} onSupply={resource=>{if(onCommand)void onCommand({type:"supplies",resource});else onUpdate(current=>reserveProvisions(current,resource));}} onFertilizer={onCommand ? null : ()=>onUpdate(current=>reserveCropFertilizer(current,1))}/><div className="settlement-balance"><span>{text.food}</span><b>{attributes.food}</b></div><div className="settlement-balance"><span>{text.water}</span><b>{attributes.water}</b></div><div className="settlement-balance"><span>{text.power}</span><b>{attributes.power}</b></div><div className="settlement-balance"><span>{text.beds}</span><b>{attributes.beds}</b></div><div className="settlement-balance"><span>{text.income}</span><b>{attributes.income}</b></div>{settlement.offlineStandalone && <section className="settlement-trade-card"><div className="pip-panel-title">{tradeText.title}</div><small>{settlement.trade?.traderAvailable ? tradeText.trader : tradeText.noTrader}</small><div className="settlement-trade-actions"><button type="button" className="pip-action-button" disabled={!canEdit || !settlement.trade?.traderAvailable || Number(attributes.food||0)<1} onClick={()=>tradeSell("food")}>{tradeText.sellFood}</button><button type="button" className="pip-action-button" disabled={!canEdit || !settlement.trade?.traderAvailable || Number(attributes.water||0)<1} onClick={()=>tradeSell("water")}>{tradeText.sellWater}</button><button type="button" className="pip-action-button" disabled={!canEdit || !settlement.trade?.traderAvailable || Number(attributes.income||0)<1} onClick={()=>tradeBuy("food")}>{tradeText.buyFood}</button><button type="button" className="pip-action-button" disabled={!canEdit || !settlement.trade?.traderAvailable || Number(attributes.income||0)<1} onClick={()=>tradeBuy("water")}>{tradeText.buyWater}</button></div><div className="settlement-balance"><span>{tradeText.purchased} · {text.food}</span><b>{Math.floor(Number(settlement.tradeReserves?.food||0))}</b></div><button type="button" className="pip-action-button" disabled={!canEdit || Number(settlement.tradeReserves?.food||0)<1} onClick={()=>usePurchasedSupply("food")}>{tradeText.use}</button><div className="settlement-balance"><span>{tradeText.purchased} · {text.water}</span><b>{Math.floor(Number(settlement.tradeReserves?.water||0))}</b></div><button type="button" className="pip-action-button" disabled={!canEdit || Number(settlement.tradeReserves?.water||0)<1} onClick={()=>usePurchasedSupply("water")}>{tradeText.use}</button><label className="settlement-trade-days">{tradeText.travel}<input className="pip-input" type="number" min="1" max="30" value={settlement.trade?.caravanTravelDays || 1} onChange={event=>setTradeTravelDays(event.target.value)}/><small>{tradeText.days}</small></label>{settlement.trade?.caravanTrip && <small>{tradeText.trip}: {settlement.trade.caravanTrip.returnDay}</small>}</section>}<div className="settlement-balance"><span>{text.common}</span><b>{Math.floor(stockpile.materials.common)}</b></div><div className="settlement-balance"><span>{text.uncommon}</span><b>{Math.floor(stockpile.materials.uncommon)}</b></div><div className="settlement-balance"><span>{text.rare}</span><b>{Math.floor(stockpile.materials.rare)}</b></div><section className="settlement-brahmin-card"><div className="pip-panel-title">{brahminText.title} · {Math.floor(Number(settlement.livestock?.brahmin||0))}</div><div className="settlement-balance"><span>{brahminText.milk}</span><b>{Math.floor(Number(stockpile.brahminMilk||0))}</b></div><div className="settlement-balance"><span>{brahminText.meat}</span><b>{Math.floor(Number(stockpile.brahminMeat||0))}</b></div><div className="settlement-balance"><span>{brahminText.fertilizer}</span><b>{Math.floor(Number(stockpile.fertilizer||0))}</b></div>{Number(settlement.livestock?.brahmin||0)>0 && <div className="settlement-brahmin-actions"><button type="button" className="pip-action-button" disabled={!canEdit || Boolean(onCommand)} onClick={()=>butcherBrahmin("food")}>{brahminText.butcherFood}</button><button type="button" className="pip-action-button" disabled={!canEdit || Boolean(onCommand)} onClick={()=>butcherBrahmin("meat")}>{brahminText.butcherMeat}</button></div>}</section><small>{Math.floor(snapshot.stockpileCapacityLbs)} lbs</small>{!settlement.offlineStandalone && <section className="settlement-profit-card"><div className="pip-panel-title">{text.playerProfit}</div><small>{text.settlementReserve}</small><div className="settlement-balance"><span>{text.caps}</span><b>{profit.claimable.caps}</b></div><div className="settlement-balance"><span>{text.food}</span><b>{profit.claimable.food}</b></div><div className="settlement-balance"><span>{text.water}</span><b>{profit.claimable.water}</b></div>{hasProfit ? <button type="button" className="pip-action-button settlement-primary" disabled={!canClaimProfit} onClick={claimProfit}>{text.claimProfit}</button> : <small>{text.profitEmpty}</small>}</section>}</div> : null}
