...
export default function SettlementScreen({ settlement, onUpdate, onBack, onCommand, canEdit=true, sharedControls, payment, canClaimProfit=false }) {
  ...
  return <div className={`pip-screen settlement-screen settlement-dashboard settlement-v2 ${onCommand ? 'is-shared-settlement' : ''} ${panelOpen ? 'is-panel-open' : ''} ${editorMode ? 'is-editor-mode' : ''}`}>
    <header className="settlement-brand"><strong>PIP 2D20 <span>/ {ui.title}</span></strong><span className="settlement-brand-name">{settlement.name}</span><button type="button" className={`pip-action-button settlement-editor-toggle ${editorMode ? "is-active" : ""}`} disabled={!canEdit} onClick={toggleEditor}><span>✥</span>{editorMode ? editor.done : editor.edit}</button><button type="button" className="pip-action-button settlement-chat-button" aria-label={ui.chat} onClick={openCampaignChat}><SheetIcon name="chat"/></button><button type="button" className="pip-action-button" aria-label={ui.more} onClick={()=>selectPanel('more')}><SheetIcon name="settings"/></button></header>
    {sharedControls}
