...
export default function SessionTacticalMapV3({session,openRequest=0}){
  ...
  useEffect(()=>{
    ...
    // A real PREP -> LIVE transition means the GM just enabled the scene.
    // Open the battlemap once for connected players, but do not reopen it for
    // ordinary scene/token updates after the player closes it.
  ...
  const overlay=open?<div className="session-tactical-overlay"><section className="pip-panel session-tactical-player"><header className="session-tactical-player__head"><div><div className="pip-bootline">PIP 2D20 // {scene.name||"TACTICAL"}</div><h2>[ TACTICAL MAP ]</h2></div><div className="session-tactical-player__actions"><span className="tactical-live">{session.status==="online"?"LIVE":"CONNECTING"}</span><button type="button" className="pip-btn" onClick={close}>BACK TO PLAYER</button></div></header>
    <div className="tactical-player-briefing-row">
      <TacticalEnvironmentSummary scene={scene} effectsOnly/>
      <div className="tactical-player-token-setup tactical-player-token-setup-v2">
        {ownedTokens.length>1?<div className="tactical-owned-token-picker">{ownedTokens.map((token)=><button type="button" key={token.id} className={`pip-btn${selectedToken?.id===token.id?" is-primary":""}`} onClick={()=>setSelectedTokenId(token.id)}>{token.avatar?<img src={token.avatar} alt=""/>:null}<span>{token.name}</span></button>)}</div>:null}
      </div>
    </div>
  ...
  </section><TacticalSessionHud session={session}/>{dragState?.moved?<div className={`tactical-drag-ghost${dragState.size===2?" is-size-2":""}`} style={{left:dragState.x,top:dragState.y}}>{dragState.avatar?<img src={dragState.avatar} alt=""/>:<b>{String(dragState.name||"T").slice(0,1)}</b>}</div>:null}</div>:null;
