...
const BASE = {
  en: {
    ...
    openSheet: "OPEN CHARACTER", players: "CONNECTED PLAYERS", workspace: "GM WORKSPACE", currentMessage: "CURRENT GM MESSAGE",
    noMessage: "No GM message yet.", sync: "SYNC CHARACTER", synced: "SYNCED", status: "STATUS",
  ...
  ru: {
    ...
    openSheet: "ОТКРЫТЬ ПЕРСОНАЖА", players: "ПОДКЛЮЧЕННЫЕ ИГРОКИ", workspace: "ПАНЕЛЬ ГМ", currentMessage: "ТЕКУЩЕЕ СООБЩЕНИЕ ГМ",
    noMessage: "ГМ пока ничего не отправил.", sync: "СИНХРОНИЗИРОВАТЬ", synced: "СИНХРОНИЗОВАНО", status: "СТАТУС",
  ...
  uk: {
    ...
    openSheet: "ВІДКРИТИ ПЕРСОНАЖА", players: "ПІДКЛЮЧЕНІ ГРАВЦІ", workspace: "ПАНЕЛЬ ГМ", currentMessage: "ПОТОЧНЕ ПОВІДОМЛЕННЯ ГМ",
    noMessage: "ГМ ще нічого не надіслав.", sync: "СИНХРОНІЗУВАТИ", synced: "СИНХРОНІЗОВАНО", status: "СТАТУС",
  ...
  pl: {
    ...
    openSheet: "OTWÓRZ POSTAĆ", players: "POŁĄCZENI GRACZE", workspace: "PANEL GM", currentMessage: "AKTUALNA WIADOMOŚĆ GM",
    noMessage: "GM nie wysłał jeszcze wiadomości.", sync: "SYNCHRONIZUJ", synced: "ZSYNCHRONIZOWANO", status: "STATUS",
      ...
      <section className="session-gm-host session-gm-host--single-workspace">
        <header className="pip-panel session-gm-hostbar">
          ...
          <div className="session-gm-hostbar__session">
            ...
            <button type="button" className="session-gm-code" onClick={handleCopy} title={copyState ? copy.copied : copy.copy}>{sessionCode}</button>
