...
const COPY = {
  en:{title:"SESSION CHAT",online:"ONLINE",connecting:"CONNECTING",offline:"OFFLINE",placeholder:"Message the group...",send:"SEND",empty:"No messages or rolls yet.",close:"CLOSE",reconnect:"RECONNECT",dice:"DICE",success:"SUCCESS",failure:"FAILURE",successes:"Suc",complications:"Comp",damage:"Damage",effects:"Effects",difficulty:"Diff",target:"TN",hit:"Hit"},
  ru:{title:"ЧАТ СЕССИИ",online:"ОНЛАЙН",connecting:"ПОДКЛЮЧЕНИЕ",offline:"ОФЛАЙН",placeholder:"Сообщение группе...",send:"ОТПРАВИТЬ",empty:"Пока нет сообщений или бросков.",close:"ЗАКРЫТЬ",reconnect:"ПЕРЕПОДКЛЮЧИТЬСЯ",dice:"КУБИКИ",success:"УСПЕХ",failure:"ПРОВАЛ",successes:"Усп",complications:"Осл",damage:"Урон",effects:"Эффекты",difficulty:"Сложн",target:"ЦЧ",hit:"Попадание"},
  uk:{title:"ЧАТ СЕСІЇ",online:"ОНЛАЙН",connecting:"ПІДКЛЮЧЕННЯ",offline:"ОФЛАЙН",placeholder:"Повідомлення групі...",send:"НАДІСЛАТИ",empty:"Ще немає повідомлень або кидків.",close:"ЗАКРИТИ",reconnect:"ПЕРЕПІДКЛЮЧИТИСЯ",dice:"КУБИКИ",success:"УСПІХ",failure:"НЕВДАЧА",successes:"Усп",complications:"Ускл",damage:"Шкода",effects:"Ефекти",difficulty:"Складн",target:"ЦЧ",hit:"Влучання"},
  pl:{title:"CZAT SESJI",online:"ONLINE",connecting:"ŁĄCZENIE",offline:"OFFLINE",placeholder:"Wiadomość do grupy...",send:"WYŚLIJ",empty:"Brak wiadomości lub rzutów.",close:"ZAMKNIJ",reconnect:"POŁĄCZ PONOWNIE",dice:"KOŚCI",success:"SUKCES",failure:"PORAŻKA",successes:"Suk",complications:"Kompl",damage:"Obrażenia",effects:"Efekty",difficulty:"Trudn",target:"TN",hit:"Trafienie"}
};
...
export default function SessionChatDrawer({session,form=null}){
  ...
  const [open,setOpen]=useState(false),[draft,setDraft]=useState(""),[diceOpen,setDiceOpen]=useState(false),[pendingAutoD6,setPendingAutoD6]=useState(null);
  ...
  return <>
    <div className={`session-chat-drawer-shell session-utility-drawer-shell${open?" is-open":""}`}>
      ...
      <aside className="session-chat-drawer session-utility-drawer" aria-hidden={!open}>
        ...
        <nav className="session-utility-tabs"><button type="button" className="pip-btn is-primary" disabled>CHAT + ROLLS</button><button type="button" className="pip-btn" onClick={()=>{setOpen(false);setDiceOpen(true);}}>{copy.dice}</button></nav>
