...
const COPY = {
  en: { title: "Campaign", battle: "Battle", world: "World", settlement: "Settlements", chat: "Chat", dice: "Dice", waiting: "Waiting for the GM’s map", hint: "Explore the shared world, visit a settlement or talk to your group while the GM prepares the scene.", character: "Character", party: "Your group" },
  ru: { title: "Кампания", battle: "Бой", world: "Мир", settlement: "Поселения", chat: "Чат", dice: "Кубики", waiting: "Ожидаем карту от ГМ", hint: "Пока ведущий готовит сцену, откройте общий мир, посетите поселение или напишите группе.", character: "Персонаж", party: "Ваша группа" },
  uk: { title: "Кампанія", battle: "Бій", world: "Світ", settlement: "Поселення", chat: "Чат", dice: "Кубики", waiting: "Очікуємо мапу від ГМ", hint: "Поки ведучий готує сцену, відкрийте спільний світ, відвідайте поселення або напишіть групі.", character: "Персонаж", party: "Ваша група" },
  pl: { title: "Kampania", battle: "Walka", world: "Świat", settlement: "Osady", chat: "Czat", dice: "Kości", waiting: "Oczekiwanie na mapę MG", hint: "Odkryj wspólny świat, odwiedź osadę lub porozmawiaj z grupą, gdy MG przygotowuje scenę.", character: "Postać", party: "Twoja drużyna" },
};
  ...
  return <section className="player-campaign player-workspace" data-workspace-tab={tab}>
    ...
    <div className="player-campaign__layout">
      ...
      <main className="player-campaign__main">
        ...
        {tab !== "battle" && <CampaignWorldMap campaignId={session.campaignId} form={form} settlementsOnly={tab === "settlement"} onOpenCampaigns={onBack} />}
      </main>
    ...
    <nav className="player-campaign__nav" aria-label={labels.title}>
      {[['battle','battle'],['world','screens'],['settlement','supplies']].map(([key, icon]) => <button key={key} type="button" aria-pressed={tab === key} onClick={() => setTab(key)}><WorkspaceIcon name={icon}/><span>{labels[key]}</span></button>)}
