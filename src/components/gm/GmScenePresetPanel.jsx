...
const RARITY_LABELS = {
  ...
  ru: { common: "Обычная", uncommon: "Необычная", rare: "Редкая", legendary: "Легендарная" },
  uk: { common: "Звичайна", uncommon: "Незвичайна", rare: "Рідкісна", legendary: "Легендарна" },
  pl: { common: "Zwykła", uncommon: "Niezwykła", rare: "Rzadka", legendary: "Legendarna" },
};
...
const COPY = {
  en: {
    ...
    lootRarity: "MAX LOOT RARITY", wealth: "LOCATION WEALTH", partyLevel: "AVG. PARTY LEVEL", partySize: "PARTY SIZE", detected: "player token(s) detected",
  ...
  ru: {
    ...
    lootRarity: "МАКС. РЕДКОСТЬ ЛУТА", wealth: "БОГАТСТВО ЛОКАЦИИ", partyLevel: "СР. УРОВЕНЬ ГРУППЫ", partySize: "РАЗМЕР ГРУППЫ", detected: "токенов игроков обнаружено",
  ...
  uk: {
    ...
    lootRarity: "МАКС. РІДКІСТЬ ЛУТУ", wealth: "БАГАТСТВО ЛОКАЦІЇ", partyLevel: "СЕР. РІВЕНЬ ГРУПИ", partySize: "РОЗМІР ГРУПИ", detected: "токенів гравців виявлено",
  ...
  pl: {
    ...
    lootRarity: "MAKS. RZADKOŚĆ ŁUPU", wealth: "BOGACTWO LOKACJI", partyLevel: "ŚR. POZIOM DRUŻYNY", partySize: "ROZMIAR DRUŻYNY", detected: "tokenów graczy wykryto",
...
function makeStartZone(cols = 12, rows = 12) {
  const result = [];
  for (let y = Math.max(0, rows - 3); y < rows; y += 1) for (let x = 0; x < Math.min(3, cols); x += 1) result.push({ x, y });
  return result;
}
...
export default function GmScenePresetPanel({ session }) {
  ...
  const [lootRarity, setLootRarity] = useState(savedSpec?.lootRarity || "uncommon");
  const [wealth, setWealth] = useState(savedSpec?.wealth || "standard");
  const [avgPartyLevel, setAvgPartyLevel] = useState(() => clampInteger(savedSpec?.avgPartyLevel, 1, 50, 1));
  const [partySize, setPartySize] = useState(() => clampInteger(savedSpec?.partySize, 1, 8, detectedPlayers || 4));
  ...
  useEffect(() => {
    ...
    if (spec) {
      ...
      setLootRarity(spec.lootRarity || "uncommon");
    ...
    } else if (detectedPlayers) {
      setPartySize(detectedPlayers);
    }
  ...
  const generationSpec = useMemo(() => ({
    type, seed, cols, rows, density: density / 100, lootRarity, wealth,
    avgPartyLevel: clampInteger(avgPartyLevel, 1, 50, 1),
    partySize: clampInteger(partySize, 1, 8, detectedPlayers || 4),
  }), [type, seed, cols, rows, density, lootRarity, wealth, avgPartyLevel, partySize, detectedPlayers]);
  ...
  const generate = async ({ newSeed = false } = {}) => {
    ...
    await session.updateTacticalScene?.({
      ...
      environment: {
        ...(scene.environment || {}),
        locationType,
        mapAssetId: `procedural:${type}:visual-v2`,
  ...
  return (
    <section className="gm-scene-presets gm-proc-map pip-panel">
      <header className="gm-scene-presets__head"><div><strong>{text.title}</strong><small>{text.subtitle}</small></div></header>
      ...
      <div className="gm-proc-map__controls">
        <label><span>{text.type}</span><select className="pip-input" value={type} onChange={(event) => setType(event.target.value)}>{MAP_TYPES.map((value) => <option key={value} value={value}>{LABELS[lang]?.[value] || LABELS.en[value]}</option>)}</select></label>
        <label><span>{text.gridSize}</span><select className="pip-input" value={gridSize} onChange={(event) => setGridSize(event.target.value)}>{GRID_SIZES.map((value) => <option key={value} value={value}>{value.replace("x", "×")}</option>)}</select></label>
        <label><span>{text.lootRarity}</span><select className="pip-input" value={lootRarity} onChange={(event) => setLootRarity(event.target.value)}>{LOOT_RARITIES.map((value) => <option key={value} value={value}>{RARITY_LABELS[lang][value]}</option>)}</select></label>
        <label><span>{text.wealth}</span><select className="pip-input" value={wealth} onChange={(event) => setWealth(event.target.value)}>{WEALTH_LEVELS.map((value) => <option key={value} value={value}>{WEALTH_LABELS[lang][value]}</option>)}</select></label>
        <label><span>{text.partyLevel}</span><input className="pip-input" type="number" min="1" max="50" value={avgPartyLevel} onChange={(event) => setAvgPartyLevel(clampInteger(event.target.value, 1, 50, 1))} /></label>
        <label><span>{text.partySize}</span><input className="pip-input" type="number" min="1" max="8" value={partySize} onChange={(event) => setPartySize(clampInteger(event.target.value, 1, 8, 4))} /><small>{detectedPlayers} {text.detected}</small></label>
        <label className="gm-proc-map__seed"><span>{text.seed}</span><div><input className="pip-input" value={seed} maxLength={40} onChange={(event) => setSeed(event.target.value)} /><button type="button" className="pip-btn" onClick={() => setSeed(makeProceduralSeed())}>{text.newSeed}</button></div></label>
        <label className="gm-proc-map__density"><span>{text.density}: {density}%</span><input type="range" min="10" max="100" step="5" value={density} onChange={(event) => setDensity(Number(event.target.value))} /></label>
      </div>
      ...
      <div className="gm-scene-presets__grid-control"><span>{text.grid}</span><div className="gm-scene-presets__grid-buttons">{["weak", "normal", "strong"].map((value) => <button key={value} type="button" className={`pip-btn${contrast === value ? " is-primary" : ""}`} onClick={() => changeContrast(value)}>{text[value]}</button>)}</div></div>
