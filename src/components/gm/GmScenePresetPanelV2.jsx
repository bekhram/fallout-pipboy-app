...
function stripLegacyEncounterOptions(spec = {}) {
  const {
    questType: _questType,
    enemyBuffsEnabled: _enemyBuffsEnabled,
    enemyBuffTier: _enemyBuffTier,
    ...clean
  } = spec || {};
  return clean;
}
      ...
      updateTacticalScene: async (patch = {}) => {
        ...
        return session.updateTacticalScene?.({
          ...
          environment: {
            ...environment,
            proceduralMapSpec: {
              ...stripLegacyEncounterOptions(proceduralMapSpec),
  ...
  const persistSetting = async (patch) => {
    ...
    await session.updateTacticalScene?.({
      environment: {
        ...(scene?.environment || {}),
        proceduralMapSpec: {
          ...stripLegacyEncounterOptions(spec),
          encounterDifficulty: difficulty,
          enemyFaction,
          enemyCountOverride,
          trapCount,
