...
function roadStyle(road, surface) {
  ...
  if (surface === "dirt") return { ...common, background: "rgba(118, 91, 58, .9)", border: "1px solid rgba(62, 45, 31, .75)", boxShadow: "inset 0 0 18px rgba(43, 31, 21, .35)" };
  if (surface === "cobblestone") return { ...common, backgroundColor: "rgba(89, 86, 79, .94)", backgroundImage: "linear-gradient(rgba(120,115,104,.38) 1px, transparent 1px), linear-gradient(90deg, rgba(55,52,48,.35) 1px, transparent 1px)", backgroundSize: "18px 14px", border: "1px solid rgba(54, 52, 47, .8)" };
  return { ...common, background: "rgba(62, 63, 60, .94)", border: "1px solid rgba(35, 36, 34, .9)", boxShadow: "inset 0 0 14px rgba(0,0,0,.28)", backgroundImage: horizontal ? "linear-gradient(to bottom, transparent 47%, rgba(174,160,111,.85) 47%, rgba(174,160,111,.85) 53%, transparent 53%)" : "linear-gradient(to right, transparent 47%, rgba(174,160,111,.85) 47%, rgba(174,160,111,.85) 53%, transparent 53%)" };
}
...
export function WastelandAssetLayer({ spec, preview = false }) {
  ...
  return (
    <div
      ...
      aria-hidden="true"
      style={{ position: "absolute", inset: 0, width: preview ? "100%" : "var(--battlemap-world-width, 100%)", height: preview ? "100%" : "var(--battlemap-world-height, 100%)", pointerEvents: "none", zIndex: preview ? 2 : 0, overflow: "hidden" }}
    >
      <img src={background} alt="" draggable={false} data-wasteland-background="true" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", pointerEvents: "none", userSelect: "none", zIndex: 0 }} />
      <div style={{ position: "absolute", inset: 0, zIndex: 1, pointerEvents: "none" }}>
        {site.roads.map((road, index) => <div key={`${road.x}-${road.y}-${road.w}-${road.h}-${index}`} data-wasteland-road={site.profile.surface} style={roadStyle(road, site.profile.surface)} />)}
      </div>
      <div style={{ position: "absolute", inset: 0, zIndex: 2, pointerEvents: "none" }}>
        {items.map((item, index) => <SpriteImage key={`${item.type}-${item.sprite ?? "x"}-${item.x}-${item.y}-${index}`} item={item} preview={preview} />)}
      </div>
    </div>
  );
}
