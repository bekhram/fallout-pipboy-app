...
function hashValue(value){const text=String(value??"");let h=2166136261;for(let i=0;i<text.length;i+=1){h^=text.charCodeAt(i);h=Math.imul(h,16777619);}return h>>>0;}
function pickRotation(seed){return[0,90,180,270][seed%4];}
function roadBounds(roads){if(!roads?.length)return null;const minX=Math.min(...roads.map(r=>Number(r.x||0)));const minY=Math.min(...roads.map(r=>Number(r.y||0)));const maxX=Math.max(...roads.map(r=>Number(r.x||0)+Number(r.w||0)));const maxY=Math.max(...roads.map(r=>Number(r.y||0)+Number(r.h||0)));return{x:minX,y:minY,w:maxX-minX,h:maxY-minY};}
function planRoadAssets(site,spec){
  const roads=site?.roads||[];if(!roads.length||site?.profile?.type==="none")return[];
  const seed=hashValue(`${spec?.seed||"1"}:${site.profile.type}:${site.terrainType}`);
  const bounds=roadBounds(roads);if(!bounds)return[];
  ...
  if(type==="cross"){
    const useT=(seed%3)===0;
    return[{src:useT?roadTJunction:roadCross,name:useT?"t-junction":"cross",x:2,y:2,w:20,h:20,rotation:pickRotation(seed)}];
  }
  if(type==="fragments"){
    return roads.map((road,index)=>{
      const rSeed=hashValue(`${seed}:${index}`),isVertical=Number(road.h||0)>=Number(road.w||0),src=index%2===0?roadDamaged:roadDeadEnd;
      const length=Math.max(Number(road.w||0),Number(road.h||0));
      const w=isVertical?8:Math.min(12,length+4),h=isVertical?Math.min(12,length+4):8;
      return{src,name:index%2===0?"damaged":"dead-end",x:Number(road.x||0)+Number(road.w||0)/2-w/2,y:Number(road.y||0)+Number(road.h||0)/2-h/2,w,h,rotation:(isVertical?0:90)+(rSeed%2?180:0)};
    });
  }
  const variant=seed%5;
  if(variant===0){
    return[{src:roadStraight,name:"straight",x:vertical?centerX-5:0,y:vertical?0:centerY-5,w:vertical?10:24,h:vertical?24:10,rotation:vertical?0:90}];
  }
  if(variant===1||variant===2){
    return[{src:variant===1?roadCurveSoft:roadCurveSharp,name:variant===1?"curve-soft":"curve-sharp",x:2,y:2,w:20,h:20,rotation:pickRotation(seed)}];
  }
  if(variant===3){
    return[{src:roadDeadEnd,name:"dead-end",x:vertical?centerX-5:1,y:vertical?1:centerY-5,w:vertical?10:22,h:vertical?22:10,rotation:(vertical?0:90)+(seed%2?180:0)}];
  }
  return[{src:roadDamaged,name:"damaged",x:vertical?centerX-5:1,y:vertical?1:centerY-5,w:vertical?10:22,h:vertical?22:10,rotation:vertical?0:90}];
}
function RoadAsset({road}){return <img src={road.src} alt="" draggable={false} data-wasteland-road={road.name} style={{position:"absolute",left:`${road.x/GRID*100}%`,top:`${road.y/GRID*100}%`,width:`${road.w/GRID*100}%`,height:`${road.h/GRID*100}%`,objectFit:"contain",transform:`rotate(${road.rotation||0}deg)`,transformOrigin:"50% 50%",pointerEvents:"none",userSelect:"none",filter:"drop-shadow(0 2px 2px rgba(0,0,0,.35))"}}/>;}
