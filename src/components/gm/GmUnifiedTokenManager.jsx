      ...
      <section className="gm-token-section">
        ...
        <div className="gm-npc-token-list">
          {npcTokens.length ? npcTokens.map((token) => {
            ...
            return <article className={`gm-npc-token-row${visible ? " is-active" : " is-hidden"}`} key={token.id}>
              ...
              <div className="gm-npc-token-row__main">
                ...
                <small>{visible ? "VISIBLE TO PLAYERS" : "HIDDEN / INACTIVE"} · INIT {Number(stats.initiative || 0)}</small>
                <div className="gm-npc-token-row__stats">
                  <span>HP {hp}/{maxHp}</span><span>DEF {Number(stats.defense || 0)}</span><span>{tokenSize(token)}×{tokenSize(token)}</span>
                </div>
              </div>
