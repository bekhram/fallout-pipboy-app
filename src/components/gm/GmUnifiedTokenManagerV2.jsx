...
function tokenSize(token) { const n = Number(token?.size); return n === 3 ? 3 : n === 2 ? 2 : 1; }
...
export default function GmUnifiedTokenManagerV2({ session }) {
  ...
  const filteredBestiary = useMemo(() => bestiaryEntries.filter(matches).slice(0, 180), [bestiaryEntries, search, typeFilter]);
  const filteredCustom = useMemo(() => library.filter(matches), [library, search, typeFilter]);
  ...
  const chooseLibrary = (id) => {
    ...
    setDraft(item ? { ...item, special: { ...(item.special || {}) }, customAttacks: [...(item.customAttacks || [])], weapons: [...(item.weapons || [])] } : blankCreature());
    setMessage("");
  };
  ...
  const setDraftRank = (patch) => {
    setDraft((current) => {
      ...
      return { ...nextBase, ...applyNpcRank(nextBase, nextBase), size: applyNpcRank(nextBase, nextBase).footprint };
    });
  };
  ...
  const addCustomAttack = () => {
    ...
    patchDraft({ customAttacks: [...(draft.customAttacks || []), normalizeStructuredAttack({ name: "New Attack", attribute: "BODY", skill: "Melee", targetNumber, damageDice: 4 })] });
  };
  ...
  const addWeapon = () => {
    ...
    if (!weapon) return;
    const melee = /melee/i.test(weapon.weaponType);
    const targetNumber = melee ? num(draft.body, 0) + num(draft.melee, 0) : num(draft.mind, 0) + num(draft.guns, 0);
    const attack = normalizeWeaponAttack({ ...weapon, targetNumber, damageDice: weapon.creatureDamage, attribute: melee ? "BODY" : "MIND", skill: melee ? "Melee" : "Guns" });
    patchDraft({ weapons: [...(draft.weapons || []), { ...attack, weaponType: weapon.weaponType, rate: weapon.rate, qualities: weapon.qualities, originalName: weapon.name }] });
    setSelectedWeaponId("");
  };
  ...
  const createBestiaryToken = async () => {
    ...
    const response = await session.createNpcToken?.({ name: selectedBestiary.name, size: stats.footprint, npcId: selectedBestiary.id, avatar: "", stats });
  ...
  const createCustomToken = async (item) => {
    ...
    const stats = buildSpawnStats(base, item.rank || spawnRank, item.hordeEnabled ?? spawnHorde, item.hordeSize || spawnHordeSize, item.specialFeature || spawnFeature);
    const response = await session.createNpcToken?.({ name: item.name, size: stats.footprint, npcId: item.id, avatar: item.avatar || "", stats });
  ...
  const selectedPreview = selectedPreviewBase ? buildSpawnStats(selectedPreviewBase, selectedBestiary ? spawnRank : selectedCustom.rank, selectedBestiary ? spawnHorde : selectedCustom.hordeEnabled, selectedBestiary ? spawnHordeSize : selectedCustom.hordeSize, selectedBestiary ? spawnFeature : selectedCustom.specialFeature) : null;
  ...
  return <section className="pip-panel gm-unified-token-manager gm-unified-token-manager-v2">
    ...
    <section className="gm-token-section gm-custom-npc-card">
      <div className="gm-token-section__head"><h3>CUSTOM NPC CARD</h3><span>ATTACKS · WEAPONS · RANK · HORDE</span></div>
      ...
      <div className="gm-npc-card-editor">
        ...
        <div className="gm-npc-card-editor__section"><div className="gm-token-section__head"><h4>CUSTOM ATTACKS</h4><button className="pip-btn" onClick={addCustomAttack}>+ ATTACK</button></div><div className="gm-npc-attack-editor-list">{(draft.customAttacks || []).map((attack, index) => <div className="gm-npc-attack-editor" key={attack.id || index}><input className="pip-input" value={attack.name || ""} placeholder="Attack name" onChange={(e) => patchCustomAttack(index, { name: e.target.value })} /><label>TN<input className="pip-input" type="number" min="0" max="20" value={attack.targetNumber ?? 0} onChange={(e) => patchCustomAttack(index, { targetNumber: Number(e.target.value) })} /></label><label>CD<input className="pip-input" type="number" min="0" value={attack.damageDice ?? 0} onChange={(e) => patchCustomAttack(index, { damageDice: Number(e.target.value) })} /></label><select className="pip-input" value={attack.skill || "Melee"} onChange={(e) => patchCustomAttack(index, { skill: e.target.value })}><option>Melee</option><option>Guns</option><option>Other</option></select><input className="pip-input" value={attack.damageType || "Physical"} placeholder="Damage type" onChange={(e) => patchCustomAttack(index, { damageType: e.target.value })} /><input className="pip-input" value={attack.effects || ""} placeholder="Effects" onChange={(e) => patchCustomAttack(index, { effects: e.target.value })} /><button className="pip-btn" onClick={() => rollAttack(attack, draftEffective, draft.name || "NPC")}>ROLL</button><button className="pip-btn" onClick={() => removeCustomAttack(index)}>×</button></div>)}</div></div>

        <div className="gm-npc-card-editor__section"><div className="gm-token-section__head"><h4>WEAPONS</h4><span>FROM WEAPON DATABASE</span></div><div className="gm-npc-weapon-add"><select className="pip-input" value={selectedWeaponId} onChange={(e) => setSelectedWeaponId(e.target.value)}><option value="">— SELECT WEAPON —</option>{weaponsDb.map((weapon) => <option value={weapon.id} key={weapon.id}>{weapon.weaponType} · {weapon.name} · {weapon.creatureDamage} CD</option>)}</select><button className="pip-btn" disabled={!selectedWeaponId} onClick={addWeapon}>ADD WEAPON</button></div><div className="gm-npc-attack-editor-list">{(draft.weapons || []).map((weapon, index) => <div className="gm-npc-weapon-row" key={weapon.id || index}><strong>{weapon.name}</strong><span>{weapon.damageDice} CD · {weapon.damageType} · {weapon.effects || "—"} · R {weapon.range || "C"}</span><label>TN<input className="pip-input" type="number" min="0" max="20" value={weapon.targetNumber ?? 0} onChange={(e) => patchWeapon(index, { targetNumber: Number(e.target.value) })} /></label><button className="pip-btn" onClick={() => rollAttack(weapon, draftEffective, draft.name || "NPC")}>ROLL</button><button className="pip-btn" onClick={() => removeWeapon(index)}>REMOVE</button></div>)}</div></div>
