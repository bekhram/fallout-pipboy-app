...
export default function CampaignSyncStatus({ world, language, authError = '' }) {
  ...
  return <section className="campaign-offline-status" aria-label={c.confirmed}>
    ...
    <div className="campaign-offline-status__dates"><span>{c.confirmed}: {stamp(world.lastSyncAt)}</span>
      {!!world.nextSyncAt && <span>{c.next}: {stamp(world.nextSyncAt)}</span>}</div>
    <div className="campaign-offline-actions">
      <button type="button" className="pip-btn" disabled={world.busy} onClick={world.syncNow}>{c.sync}</button>
      <button type="button" className="pip-btn" disabled={!world.campaign} onClick={world.exportSave}>{c.export}</button>
      {navigator.storage?.persist && <button type="button" className="pip-btn" onClick={async () => {
    ...
    {!!rejected.length && <details className="campaign-offline-review"><summary>{c.review} · {rejected.length} {c.rejected.toLowerCase()}</summary>
      {rejected.slice(0, 10).map(entry => <p key={entry.requestId}>{c.rejected} #{entry.sequence || entry.requestId.slice(0, 8)}: {offlineError(entry.error, language, worldError(entry.error, worldCopy(language)))}</p>)}
    </details>}
  </section>;
}
