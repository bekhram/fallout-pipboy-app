...
export default function CampaignWorldMap({ campaignId, form, onOpenCampaigns, settlementsOnly = false }) {
  ...
  return <section className="campaign-world" aria-label={c.title}>
    ...
    {characterPanel}
    {active && createPortal(<SettlementScreen key={active.id} settlement={active} character={campaign.character || null} onBack={()=>setActiveId(null)} sharedControls={settlementControls} canEdit={Boolean(editable)&&!localDisabled} onCommand={async command=>{if(localDisabled||!editable)return false;return Boolean(await run({type:'settlement',settlementId:active.id,command}));}}/>,document.body)}
  </section>;
}
