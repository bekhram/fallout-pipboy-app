      ...
      class MapScene extends Phaser.Scene {
        ...
        refresh() {
          ...
          for (const m of d.markers) {
            ...
            const markerColors = { danger:0xff8f8f, loot:0xffdc73, quest:0x7fdcff, objective:0xffb86c, settlement:0x9dff9d, note:0xa9e5a6 };
            const markerColor = m.kind === 'member' ? 0x62d9ff : m.visibility === 'gm' ? 0xffd166 : (markerColors[m.category] || 0xa9e5a6);
            const textColor = m.kind === 'member' ? '#8fe7ff' : m.visibility === 'gm' ? '#fff1a8' : '#c5eebe';
            g.fillStyle(0x08180d, .95).fillCircle(x, y, radius).lineStyle(2 / this.cameras.main.zoom, markerColor).strokeCircle(x, y, radius);
            this.labels.push(this.add.text(x, y, m.icon || '◆', { fontFamily: 'monospace', fontSize: `${Math.max(22, 17 / this.cameras.main.zoom)}px`, color: textColor }).setOrigin(.5).setDepth(51));
            if (m.label) {
