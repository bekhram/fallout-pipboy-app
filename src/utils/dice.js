...
export function rollHitLocationD20() {
  ...
  return { type: "hit-location-d20", value, location, label: getHitLocationLabel(location), timestamp: new Date().toISOString() };
}

export function rollSingleDie(sides) { return Math.floor(Math.random() * sides) + 1; }
...
export function evaluateFalloutD20Value(value, { targetNumber = null, criticalRange = 1 } = {}) {
  ...
  const safeTargetNumber = hasTarget ? clamp(Number(targetNumber), 0, 20) : null;
  ...
  if (safeTargetNumber !== null) {
    if (value <= safeTargetNumber) {
      isSuccess = true;
      if (value <= safeCriticalRange) { successes = 2; isCritical = true; }
      else successes = 1;
    }
  } else if (value <= safeCriticalRange) {
    successes = 2;
    isCritical = true;
  }
  return { value, successes, isSuccess, isCritical, isComplication };
}

export function createSkillRoll({ title, skillName, skill, testValue, diceCount = 2, difficulty = 1 }) {
  const skillRank = Number(skill?.rank || 0);
  const isTagged = !!skill?.tagged;
  return { type: "skill", diceType: "d20", title, skillName, skill, targetNumber: testValue, criticalRange: isTagged ? clamp(skillRank, 1, 20) : 1, diceCount, difficulty };
}

export function buildFalloutD20Result(rollValues, { targetNumber = null, criticalRange = 1, label = "" } = {}) {
  const rolls = rollValues.map((value) => evaluateFalloutD20Value(value, { targetNumber, criticalRange }));
  return {
    type: "fallout-d20-basic",
    label,
    diceCount: rolls.length,
    targetNumber: targetNumber === null || targetNumber === undefined ? null : Number(targetNumber),
...
export function rerollOneFalloutD20(result, dieIndex, { targetNumber, criticalRange, label } = {}) {
  if (!result?.rolls?.length) return result;
  if (dieIndex < 0 || dieIndex >= result.rolls.length) return result;
  const nextValues = result.rolls.map((die, index) => index === dieIndex ? rollSingleDie(20) : die.value);
  return buildFalloutD20Result(nextValues, {
    targetNumber: targetNumber !== undefined ? targetNumber : result?.targetNumber ?? null,
    criticalRange: criticalRange !== undefined ? criticalRange : result?.criticalRange ?? 1,
...
export function evaluateFalloutD6Value(value) {
  ...
  if (value === 2) return { value, damage: 2, effect: 0, label: "2" };
  if (value === 3 || value === 4) return { value, damage: 0, effect: 0, label: "0" };
  if (value === 5 || value === 6) return { value, damage: 1, effect: 1, label: "1+" };
...
export function buildFalloutD6Result(rollValues, { effects = [] } = {}) {
  ...
  const processedRolls = rolls.map((die) => {
    ...
    if (hasVicious && die.label === "1+") finalDamage = 2;
    totalDamage += finalDamage;
    return { ...die, normalDamage, damage: finalDamage, viciousBonus: finalDamage - normalDamage };
  });
  ...
  if (hasSpread && totalEffects > 0 && spreadDamagePerHit > 0) {
    for (let i = 0; i < totalEffects; i += 1) {
      ...
      spreadHits.push({ location: hit.location, label: hit.label, roll: hit.value, damage: spreadDamagePerHit });
    }
  }
  const burstTargets = [];
...
export function rerollOneFalloutD6(result, dieIndex, { effects } = {}) {
  if (!result?.rolls?.length) return result;
  if (dieIndex < 0 || dieIndex >= result.rolls.length) return result;
  const nextValues = result.rolls.map((die, index) => index === dieIndex ? rollSingleDie(6) : die.value);
  return buildFalloutD6Result(nextValues, { effects: effects !== undefined ? effects : result?.rawEffects || result?.effects || [] });
}

export function createWeaponRoll({ weapon, diceCount = 2, difficulty = 1, useRate = false }) {
  return {
    type: "weapon",
    diceType: "d20",
