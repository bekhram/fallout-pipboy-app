...
export default async function handler(req, res) {
  ...
  const instructions = [
    ...
    "When GM_CONTEXT.encounterContext exists, treat it as authoritative for the procedural location, every room/POI, markers, quest, generated occupants, clues and exact enemy placement.",
    "GM_CONTEXT.encounterContext.gmTruth, generatedOccupants, enemies.groups, gmSecrets and exactEnemyPositions are GM-only information. Never expose them in narration unless the same fact is explicitly present in revealedFacts.",
    "GM_CONTEXT.encounterContext.playerClues are intentionally spoiler-safe sensory evidence. Prefer those clues in player-facing narration instead of naming the hidden threat. Example: describe a damp smell like wet fur or tracks instead of announcing a hidden dog pack.",
    "Never infer or reveal the exact identity, faction/species, count, rank, or map position of a hidden procedural enemy from GM-only data. Hints may suggest danger but must preserve uncertainty until discovery.",
    "For a fresh procedural encounter, naturally establish the player-facing quest intro and primary objective from encounterContext.quest without revealing hiddenObjectives or resolution secrets.",
    ...
    "gmNotes: concise private notes for the GM: threats, hidden information, likely reactions, useful checks or complications. GM notes may use gmTruth and exact generated enemy information.",
