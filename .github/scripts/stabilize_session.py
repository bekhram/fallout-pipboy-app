...
p = Path('src/hooks/useSharedSession.js')
s = p.read_text()
...
s = s.replace('return value.length <= 220000 ? value : "";', 'return value.length <= 80000 ? value : "";')

old_packet = '''function makePlayerPacket(name, form) {
  return {
    name: String(name || "Player").trim().slice(0, 40) || "Player",
    character: createCharacterSnapshot(form),
  };
}'''
new_packet = '''function createLiveCharacterSnapshot(form) {
...
if old_packet not in s:
    raise SystemExit('makePlayerPacket block not found')
s = s.replace(old_packet, new_packet)

marker = 'export default function useSharedSession(form) {'
head, sep, _ = s.partition(marker)
if not sep:
    raise SystemExit('hook marker not found')

hook = r'''export default function useSharedSession(form) {
